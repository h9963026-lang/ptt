...
# 诈骗帖子统计文件（包含看板名）
STATS_EXCEL_PATH = str(BASE_DIR / "诈骗帖子统计 2.xlsx")
# 存储路径（在脚本所在目录下创建“看板数据”文件夹）
STORAGE_DIR = str(BASE_DIR / "看板数据")
# 输出文件名称
...
def sanitize_html(html: str) -> str:
    ...
    try:
        ...
        pass
    pattern = re.compile(
        r"<script[^>]*>.*?ask/over18.*?</script>", re.DOTALL | re.IGNORECASE
    )
...
def append_raw_record(
    path: Optional[str], record: Dict[str, Any], compress: bool = False
) -> None:
    if not path:
        return
...
def get_board_names_from_excel() -> List[str]:
    """从《诈骗帖子统计 2.xlsx》中读取看板名称（使用 board 列）"""
    try:
        df = pd.read_excel(STATS_EXCEL_PATH)
        if "board" not in df.columns:
            print("诈骗帖子统计文件中未找到“board”列，请检查表头！")
            return []
        board_names = df["board"].dropna().unique().tolist()
        print(f"从诈骗帖子统计文件成功读取 {len(board_names)} 个看板名称")
        return [str(b) for b in board_names]
    except Exception as e:
        print(f"读取诈骗帖子统计文件失败：{str(e)}")
        return []
...
def crawl_boards(
    ...
    raw_urls_seen: Set[str] = (
        existing_raw_urls if existing_raw_urls is not None else set()
    )
    ...
    for board in boards:
        ...
        if use_search and not full_index_scan:
            if search_pages <= 0:
                ...
                for keyword in normalized_keywords or [""]:
                    ...
                    for soup in crawler.iter_search_pages(
                        board, keyword, search_pages, page_sleep
                    ):
                        page_idx += 1
                        ...
                        for p in posts:
                            ...
                            if not url or url in seen_urls:
                                continue
                            title = p.get("title", "")
                            if keyword and match_scope == "title" and not PTTCrawler.match_keywords(
                                title, [keyword]
                            ):
                                continue
                            seen_urls.add(url)
        ...
        # 2) 若搜索无结果，遍历看板索引页
        need_index_scan = full_index_scan or (
            not candidate_posts and fallback_index_pages > 0
        )
        if need_index_scan:
            ...
            for soup in crawler.iter_board_pages(board, max_pages=limit):
                ...
                for p in posts:
                    ...
                    if not url or url in seen_urls:
                        continue
                    title = p.get("title", "")
                    if normalized_keywords and match_scope == "title" and not PTTCrawler.match_keywords(
                        title, normalized_keywords
                    ):
                        continue
                    seen_urls.add(url)
                    candidate_posts.append(p)
        ...
        for p in candidate_posts:
            if max_downloads_per_batch and saved >= max_downloads_per_batch:
                print(
                    f"  - 已达到每批下载上限 ({max_downloads_per_batch})，暂停当前看板"
                )
                break
            ...
            # 不再单独保存HTML文件，只保留元数据和JSONL中的原始HTML
...
def main() -> None:
    ...
    for keyword_index, keyword_list in enumerate(keyword_groups, start=1):
        ...
        for batch_index in range(total_batches):
            ...
            new_records = crawl_boards(
                ...
                out_dir=os.path.join(STORAGE_DIR, "ptt_html"),  # 仍传入子目录名，但不再写入HTML
                max_pages=10,
                delay=1.2,
                timeout=15.0,
                skip_existing=True,
    ...
    print(f"  - 原始数据JSONL（含HTML文本）：{RAW_JSONL}")
    print("  - 不再单独生成每篇文章的HTML文件")
